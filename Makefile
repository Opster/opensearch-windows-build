...
build:
	...
	aws s3 cp opensearch-$(VERSION)-win-source.zip s3://opensearch-win/
democerts:
	...
	aws s3 cp opensearch-$(VERSION)-with-democerts.zip s3://opensearch-win/
cleancerts:
...
dashboard:
	...
	aws s3 cp opensearch-dashboards-$(VERSION)-SNAPSHOT-windows-x64.zip s3://opensearch-win/
dashboard-plugins:
	...
	aws s3 cp dashboard-plugins-$(VERSION).zip s3://opensearch-win/
clean:
